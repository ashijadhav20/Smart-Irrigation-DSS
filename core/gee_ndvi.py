...
def make_query_signature(latitude, longitude, evaluation_date, area_m2):
...
def _sampling_region(ee, latitude, longitude, area_m2=None, radius_m=100):
    ...
    if area_m2 and float(area_m2) > 0:
        ...
        method = "Equivalent-area circular field footprint"
    else:
        radius = max(20.0, float(radius_m))
        method = "Circular sampling footprint"
...
def get_sentinel2_ndvi(
    ...
    region, used_radius, footprint_method = _sampling_region(
        ee, latitude, longitude, area_m2=area_m2, radius_m=radius_m
    )
    ...
    return {
        ...
        "sampling_radius_m": float(used_radius),
        "footprint_method": footprint_method,
        "query_signature": make_query_signature(
            latitude, longitude, evaluation, area_m2
        ),
        "status": "clear",
